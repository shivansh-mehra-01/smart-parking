...
def init_db(db_path="parking.db"):
...
def record_entry(conn, plate, source):
    cur = conn.cursor()
    cur.execute(
        "SELECT id FROM parking_log WHERE plate_text=? AND status='inside'",
        (plate,)
    )
    if cur.fetchone():
        print(f"  [Entry] ⚠️  {plate} pehle se andar hai!")
        return False
    cur.execute(
        "INSERT INTO parking_log (plate_text, entry_time, status, source) VALUES (?,?,?,?)",
        (plate, datetime.now().isoformat(), 'inside', source)
    )
    conn.commit()
...
def record_exit(conn, plate, source):
    ...
    cur.execute(
        """SELECT id, entry_time FROM parking_log
           WHERE plate_text=? AND status='inside'
           ORDER BY id DESC LIMIT 1""",
        (plate,)
    )
    row = cur.fetchone()
    if not row:
        print(f"  [Exit] ⚠️  {plate} ki entry nahi mili!")
        return None

    rec_id, entry_str = row
    entry_dt  = datetime.fromisoformat(entry_str)
    exit_dt   = datetime.now()
    dur_mins  = (exit_dt - entry_dt).total_seconds() / 60.0

    cur.execute(
        "UPDATE parking_log SET exit_time=?, duration_mins=?, status='exited' WHERE id=?",
        (exit_dt.isoformat(), round(dur_mins, 2), rec_id)
    )
    conn.commit()
                    ...
                    if is_duplicate:
                        ...
                        if final_conf > old_conf:
                            ...
                            if is_entry and old_db_id is not None:
                                cur = conn.cursor()
                                cur.execute("UPDATE parking_log SET plate_text=? WHERE id=?", (final_text, old_db_id))
                                conn.commit()
...
def main():
    ...
    conn.close()
    print("\n✅ Dono cameras complete. Data 'parking.db' mein saved!")
